...
- [x] `src/platform/streamdeck/settings/punch-settings.ts` を読み、`KotPunchSettings` 型の現行定義を確認する
- [x] `src/services/kot/auth.ts` を読み、`puppeteer.launch()` のオプション構造を確認する
- [x] `com.hrk-m.kot-punch.sdPlugin/ui/clock-in.html` / `clock-out.html` を読み、`sdpi-components` の既存コンポーネントパターンを確認する
- [x] `src/services/kot/__tests__/auth.test.ts` を読み、`puppeteer.launch` のモック方法を確認する
- [ ]* 全体コードを俯瞰する（任意）

### Phase 1.2: 変更候補を特定する

- [x] 以下の変更ファイルと変更理由を確認する:
...
- [x] `docs/spec/punch.md` の「ボタン・アイコン一覧」と「依存関係」に変更が不要であることを確認する（UI/icon への影響なし）
- [x] **CHECKPOINT**: 変更対象 5 ファイルと影響範囲（出勤・退勤打刻のみ、Open KOT / Open Request への影響なし）が明確
...
- [x] `KotPunchSettings` に `kotPunchHeadless?: boolean` を追加し、型定義を確定する

### Phase 2.2: 空モックで全体を成立させる

- [x] `com.hrk-m.kot-punch.sdPlugin/ui/clock-in.html` に `sdpi-checkbox` を追加する:
...
- [x] `com.hrk-m.kot-punch.sdPlugin/ui/clock-out.html` に同一 `sdpi-checkbox` を追加する（`setting="kotPunchHeadless"` は同じグローバル設定キー）

- [x] Stream Deck を再起動してチェックボックスが Property Inspector に表示されることを手動確認する
...
- [x] **`**MOCK-IMPL**`**: `auth.test.ts` に headless 設定の検証テストを追加した（Puppeteer 実起動なし・3 ケース）

- [x] `bun run test` で既存テストがすべてパスし、追加テストも通ることを確認する（110 tests passed）

- [x] **CHECKPOINT**: 実 Puppeteer 起動なし + モックで headless オプションの分岐が end-to-end で検証可能
...
- [x] Phase 2 で成立させたモックと型契約を元に、残り作業を確認する

### Phase 3.2: 重要処理から穴埋めする

- [x] `src/services/kot/auth.ts` の `puppeteer.launch()` を変更した

- [x] `bun run lint && bun run test && bunx tsc --noEmit && bun run build` を実行してすべてパスすることを確認した

- [x] 手動動作確認（Stream Deck 実機で確認）:
...
- [x] **CHECKPOINT**: ヘッドレス ON/OFF の主要ユースケースがモック + 実動作で再現可能
