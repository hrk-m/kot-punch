...
- [ ] `src/actions/open-kot.ts` の既存パターンを読む（`onKeyUp` / `_isProcessing` ガード / エラーハンドリング）
- [ ] `src/lib/settings.ts` の設定型・必須チェック関数パターンを確認する
- [ ] `src/lib/puppeteer.ts` の `openKotPage()` / `punchKot()` 構造を確認する
- [ ] `src/plugin.ts` の `registerAction` 登録パターンを確認する
- [ ] `manifest.template.json` の `Actions` 配列の構造を確認する
- [ ] 既存 Property Inspector HTML の構造を確認する

### Phase 1.2: 変更候補を特定する

- [ ] 影響/改修ファイルを列挙し、変更理由を整理する
...
- [ ] **MOCK-CONTRACT**: `RequestSettings` 型を確定する
  ```typescript
...
- [ ] `src/actions/open-request.ts` を作成し、`onKeyUp` の基本制御を実装する
- [ ] `src/lib/settings.ts` に `RequestSettings` と必須チェック関数を追加する
- [ ] `src/lib/puppeteer.ts` に `openRequestPage()` スタブを追加する
- [ ] `src/plugin.ts` に `OpenRequest` の `registerAction` を追加する
- [ ] `manifest.template.json` に `open-request` エントリを追加し `bun run generate-manifest` を実行する
- [ ] `com.hrk-m.kot-punch.sdPlugin/ui/open-request.html` を作成する
- [ ] アイコン画像を `imgs/actions/open-request/` に配置する
- [ ] `bun run build` が通ることを確認する
- [ ] **CHECKPOINT**: 空モックで end-to-end の経路が成立
...
- [ ] `src/lib/puppeteer.ts` の `openRequestPage()` を実装する（TDD: RED → GREEN）
...
- [ ] `src/actions/__tests__/open-request.test.ts` を作成する
  - テストケース 8 件: 成功 / 成功後リセット / 設定なし / 設定なし後リセット / エラー表示 / エラー後リセット / 連打防止 / 引数検証
- [ ] `src/lib/__tests__/settings-request.test.ts` を追加する
  - `getRequestSettings()` が `requestUrl` / `requestUsername` / `requestPassword` を返すことを検証
- [ ] `src/lib/__tests__/settings.test.ts` で `hasRequiredRequestSettings()` を検証する
- [ ] `src/lib/__tests__/puppeteer.test.ts` で `openRequestPage()` の操作順序・入力値・例外時クリーンアップを検証する
- [ ] `bun run lint && bun run test && bunx tsc --noEmit && bun run build` が通ることを確認する（69 tests pass）
- [ ] **CHECKPOINT**: 主要ユースケースが自動テストで再現可能
