...
- [x] `src/actions/clock-out.ts` を読む（旧実装の構造・削除対象を把握）
- [x] `src/actions/clock-in.ts` を読む（新パターンを把握し、差分箇所を特定）
- [x] `src/actions/__tests__/clock-out.test.ts` を読む（旧テストの構造・削除対象を把握）
- [x] `src/lib/puppeteer.ts` を読む（`punchKot("#leave")` の実装済み状態を確認）
- [x] `src/lib/settings.ts` を読む（`username` / `password` / `dryRun` が追加済みであることを確認）
...
- [x] **MOCK-CONTRACT**: `ClockOut` の状態遷移を確定する
...
- [x] **MOCK-CONTRACT**: `onKeyUp` 呼び出しシグネチャを確定する（`clock-in.ts` と同一）
  ```typescript
  override async onKeyUp(ev: KeyUpEvent): Promise<void>
  // ev.payload.state: 0 | 1
  // ev.action: { setState, showOk, showAlert }
  ```
- [x] **MOCK-CONTRACT**: テストで使う共通 action スタブを確定する
  ```typescript
...
- [x] **MOCK-IMPL**: `src/actions/clock-out.ts` を全面書き換えする
...
- [x] **MOCK-IMPL**: `src/actions/__tests__/clock-out.test.ts` を全面書き換えする
...
- [x] `bun run build` でビルドが通ることを確認する
- [x] `bun run test` で既存テストが pass することを確認する（44 tests passed）
- [x] **CHECKPOINT**: モック状態で end-to-end 実行可能。State 0 → 押す → State 1、State 1 → 押す → State 0 が確認できる
...
- [x] 連打防止フロー: `_isProcessing=true` のとき `onKeyUp` が即 return し `setState` / `punchKot` が呼ばれないことを検証
  - 操作前提: `_isProcessing=true`（処理中状態）/ 操作: `onKeyUp` / 期待結果: 何も呼ばれない
- [x] State 0 打刻成功フロー: `punchKot` モックが resolve のとき `showOk()` + `setState(1)` が呼ばれることを検証
  - 操作前提: `state=0`, `_isProcessing=false`, 設定あり / 操作: `onKeyUp` / 期待結果: `showOk` + `setState(1)` 呼び出し
- [x] State 0 打刻失敗フロー: `punchKot` がエラーを throw したとき `showErrorImage()` + `setState(0)` が呼ばれることを検証
  - 操作前提: `state=0`, `punchKot` が throw / 操作: `onKeyUp` / 期待結果: `showErrorImage` + `setState(0)` 呼び出し
- [x] State 1 リセットフロー: State 1 でボタンを押すと `setState(0)` が呼ばれ `punchKot` は起動しないことを検証
  - 操作前提: `state=1` / 操作: `onKeyUp` / 期待結果: `setState(0)` のみ。`punchKot` は呼ばれない
- [x] 設定未完了フロー: `hasRequiredPunchSettings=false` のとき `showAlert()` が呼ばれ `punchKot` は起動しないことを検証
...
- [x] `#leave` テストは `puppeteer.test.ts` line 222-228 にすでに存在していたため追記不要
  - `punchKot("#leave", { ...settings, dryRun: true })` → `click("#leave")` が呼ばれることを検証済み

#### 3.2.3 最終検証

- [x] `bun run test` で全ユニットテスト pass を確認する（44 tests passed）
