...
- [ ] `bun run logs` でリアルタイムログを監視し、エラーメッセージを記録する
...
- [ ] エラーが `punchKot` 関数のどのステップで発生しているかをログから特定する
...
- [ ] 参照実装 `yuyakinjo/attend-kingoftime/src/punch-script.ts` と照合して正しいセレクタを確認する
...
- [ ] `makePage()` に `waitForSelector` / `evaluate` を追加する（`puppeteer.test.ts`）
- [ ] `resetMocks()` に `mockWaitForSelector` / `mockEvaluate` のリセット処理を追加する
...
- [ ] `src/lib/puppeteer.ts` のインライン `wait` 関数を `import { setTimeout as sleep } from "node:timers/promises"` に置き換える
- [ ] テスト内で `mockSleep` の呼び出し回数と引数をアサートできるようにする
- [ ] **CHECKPOINT**: モックで `punchKot` の全ステップが end-to-end 実行可能
...
- [ ] `page.click("button[type=submit]")` + `waitForNavigation` を削除する
- [ ] 500ms 待機 → `page.evaluate('document.querySelector("[type=submit]")?.click()')` → 1000ms 待機 に変更する
  - **操作前提**: パスワード入力完了済み
...
- [ ] ユーザー選択: `::-p-text(username)` → `[title*='username']` に変更する
  - `waitForSelector([value*='username'])` + 500ms 待機を追加
- [ ] パスワード入力: `input[type=password]` → `.input_password` に変更する
  - `waitForSelector('#password_dialog')` + 500ms 待機を追加
- [ ] 打刻ボタン: クリック前に `waitForSelector(selector)` を追加する

#### テストを新セレクタに対応させる

- [ ] アサーションを新セレクタに更新する
  - `click([title*='username'])` / `waitForSelector('#attend')` 等
- [ ] `mockSleep` の呼び出し順序・引数をテストに追加する
  - `(500, 500, 500, 1000)` の順
- [ ] dryRun=true 時に `mockEvaluate` が呼ばれないことをアサートする
- [ ] **CHECKPOINT**: 全 72 テスト通過・lint / 型チェック / ビルドすべて成功
