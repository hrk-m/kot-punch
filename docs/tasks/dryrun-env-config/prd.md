...
1. **As a developer**, I want to set `KOT_PUNCH_DRY_RUN=true` in `.env` so that I can verify the punch flow without actually submitting.
...
- [ ] `punchKot()` が `process.env.KOT_PUNCH_DRY_RUN === "true"` を dryRun フラグとして使用している
- [ ] テストで `process.env.KOT_PUNCH_DRY_RUN` を設定することで dryRun 分岐をカバーできる
...
.env (プロジェクトルート, gitignore 対象)
  KOT_PUNCH_DRY_RUN=true|false
          │
          │ bun run build 時に Bun が自動読み込み
          ↓
rollup.config.mjs (@rollup/plugin-replace)
  process.env.KOT_PUNCH_DRY_RUN → "true" or "false" に inline 展開
          │
          ↓
...
| `src/services/kot/punch.ts` | 編集 | `settings.kotPunchDryRun` を `process.env.KOT_PUNCH_DRY_RUN === "true"` に置き換え |
...
| `src/services/kot/__tests__/punch.test.ts` | 編集 | `kotPunchDryRun` を `process.env.KOT_PUNCH_DRY_RUN` の設定・クリーンアップに変更 |
...
`bun run build` 実行時に Bun がプロジェクトルートの `.env` を自動読み込みするため、Rollup config 内で `process.env.KOT_PUNCH_DRY_RUN` が参照可能になる。
