...
出勤・退勤ボタンの両方に 3 秒長押し判定を追加し、長押し時だけ `State 0 <-> State 1` を手動更新できるようにする。短押しは既存の打刻操作を維持し、`State 1` の短押しは no-op に変更する。

### Success Criteria

- `ClockIn` / `ClockOut` の両方で、3 秒以上の長押し後にキーを離すと `State 0 <-> State 1` が切り替わる
...
| 2 | ユーザーとして、打刻を実行せずに見た目 state を手動で `State 1` に合わせたい | `State 0` で 3 秒以上長押しして離すと `setState(1)` のみ実行され、Puppeteer と通知は呼ばれない |
| 3 | ユーザーとして、打刻済み表示を誤タップで解除したくない | `State 1` の短押しでは何も起きず、3 秒以上長押しして離したときだけ `setState(0)` が実行される |
...
| Stream Deck 上の `Clock In` / `Clock Out` ボタン（State 0） | 3 秒長押しして離す | 同一ボタン | 即時に State 1 |
| Stream Deck 上の `Clock In` / `Clock Out` ボタン（State 1） | 短押し | 同一ボタン | State 1 のまま変化なし |
| Stream Deck 上の `Clock In` / `Clock Out` ボタン（State 1） | 3 秒長押しして離す | 同一ボタン | 即時に State 0 |
...
3. 押下時間が `3000ms` 以上なら現在の `ev.payload.state` を見て `setState(0|1)` を実行して終了する
4. 押下時間が `3000ms` 未満かつ `State 0` なら既存の打刻処理へ進む
5. 押下時間が `3000ms` 未満かつ `State 1` なら no-op で終了する
...
| 現状 | 変更後 |
|------|--------|
| `State 1` の短押しで即 `setState(0)` | `State 1` の短押しは no-op、3 秒長押し時のみ `setState(0)` |
| `State 0` は短押しで打刻成功後に `setState(1)` | 維持。加えて 3 秒長押しで `setState(1)` を手動実行可能にする |
...
- `clock-in.test.ts` / `clock-out.test.ts` で `onKeyDown` を追加モックし、`vi.useFakeTimers()` で 2999ms と 3000ms の境界を検証する
...
- MVP: `ClockIn` / `ClockOut` に 3 秒長押し state 更新を追加し、既存短押し打刻を維持する
...
| 3000ms 境界の判定が不安定になる | fake timer で 2999ms / 3000ms の境界ケースを明示的に回帰テストする |
